...
if [ "$fish_key_bindings" = fish_vi_key_bindings ]; bind -Minsert ! __history_previous_command bind -Minsert '$' __history_previous_command_arguments
else
...
if test -d ~/.cache/starship
   ...
   set -l latest_logs (ls -t ~/.cache/starship/session_*.log 2>/dev/null | head -n 5)
   test -n "$latest_logs"; or return
...
case '.tar.xz' '.txz'
tar xf $file
case '.tar.zst' tar --zstd -xf $file case '.bz2'
bunzip2 $file
case '.rar' unrar x $file case '.gz'
gunzip $file
case '.tar' tar xf $file case '.zip'
unzip $file
case '.Z' uncompress $file case '.7z'
7z x $file
case '*'
